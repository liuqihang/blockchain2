...
import (
	...
	"github.com/cosmos/ethermint/ethereum"
	emtTypes "github.com/cosmos/ethermint/types"
...
func NewEthermintApplication(backend *ethereum.Backend,
	...
	stat, err := backend.Ethereum().BlockChain().State()
	if err != nil {
		return nil, err
	}
...
func (app *EthermintApplication) Query(query abciTypes.RequestQuery) abciTypes.ResponseQuery {
	...
	if err := json.Unmarshal(query.Data, &in); err != nil {
		return abciTypes.ResponseQuery{Code: 90,
			Log: err.Error()}
	}
	var result interface{}
	if err := app.rpcClient.Call(&result, in.Method, in.Params...); err != nil {
		return abciTypes.ResponseQuery{Code: 91,
			Log: err.Error()}
	}
	bytes, err := json.Marshal(result)
	if err != nil {
		return abciTypes.ResponseQuery{Code: 92,
			Log: err.Error()}
	}
