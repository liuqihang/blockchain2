...
func (ms *ManagedState) SetState(statedb *CommitStateDB) {
	ms.mu.Lock()
	defer ms.mu.Unlock()
	ms.CommitStateDB = statedb
}
...
func (ms *ManagedState) GetNonce(addr common.Address) uint64 {
	...
	if ms.hasAccount(addr) {
		...
		return ms.CommitStateDB.GetNonce(addr)
	}
}
...
func (ms *ManagedState) getAccount(addr common.Address) *account {
	if account, ok := ms.accounts[addr]; !ok {
		...
		so := ms.CommitStateDB.getStateObject(addr)
